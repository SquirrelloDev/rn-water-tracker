    ...
    return (
        <StyledView className={'my-4'}>
            <StyledView className={'my-1'}>
                <StyledText className={'font-medium text-base px-3 dark:text-white'}>Interwał powiadomień</StyledText>
            </StyledView>
            <StyledView className={'flex-row flex-wrap items-center justify-center bg-slate-100 dark:bg-neutral-950 rounded-xl m-3'}>
            {intervals.map(interval => (
                <StyledPressable key={interval.title} className={'w-40 grow'} onPress={() => {
                    ...
                    <Card classNames={clsx(interval.value === waterNotificationInterval && 'bg-sky-400')}>
                        <StyledText className={clsx('text-center font-medium dark:text-white', interval.value === waterNotificationInterval && 'text-white')}>{interval.title}</StyledText>
                    </Card>
                </StyledPressable>
            ))}
            </StyledView>
